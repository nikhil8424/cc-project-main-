...
export async function uploadMultiplePosts(data) {
    const { title, description, files, postedBy, tag } = data;
    
    console.log('uploadMultiplePosts - Files to process:', files.length);
    console.log('uploadMultiplePosts - Files:', files.map(f => f.name || 'unnamed'));
    
    try {
        // Filter out any non-file objects and ensure we have valid files
        const validFiles = files.filter(file => file && file.name && file.size > 0);
        console.log('uploadMultiplePosts - Valid files:', validFiles.length);
        
        if (validFiles.length === 0) {
            throw new Error('No valid files to upload');
        }
        
        // Process each file and create a post
        for (let i = 0; i < validFiles.length; i++) {
            const file = validFiles[i];
            console.log(`Processing file ${i + 1}/${validFiles.length}:`, file.name);
            
            // Create unique title for each image by adding index or using filename
            const uniqueTitle = validFiles.length > 1 
                ? `${title} - ${file.name.split('.')[0]}` 
                : title;
            
            // Create unique storage reference to avoid conflicts
            const timestamp = Date.now();
            const fileName = `${timestamp}_${file.name}`;
            const storageRef = firebase
                .storage()
                .refFromURL("gs://pointerest-4877f.appspot.com/" + fileName);
            ...
            await updatePostId(newPostRef);
            console.log('Post created successfully for:', file.name);
        }
        
        // Increment post count by the number of files uploaded
        for (let i = 0; i < validFiles.length; i++) {
            incrementPostCount(postedBy);
        }
        
        console.log(`Successfully uploaded ${validFiles.length} posts`);
    } catch (error) {
        console.error("Error uploading multiple posts:", error);
        throw error;
    }
}
